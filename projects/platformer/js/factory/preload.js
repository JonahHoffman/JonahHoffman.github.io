(function (window) {
    'use strict';
    window.opspark = window.opspark || {};
    let opspark = window.opspark;
    
    // TODO : Load config for url //
    opspark.preload = function (game) {
        game.load.image('cannon', './asset/cannon.png');
        game.load.image('projectile', './asset/yellowdollarsign.png');
        game.load.image('platform', './asset/pewdplatform.jpg');
        game.load.image('db', './asset/collectable/database.png');
        game.load.image('steve', './asset/collectable/subscribe.png');
        game.load.image('grace', './asset/collectable/monetization.png');
        game.load.image('kennedi', './asset/collectable/chair.png');
        game.load.image('max', './asset/collectable/max-head.png');
        game.load.image('sub2pewds', './asset/Pewdiepie.jpg');
        game.load.atlas('halle', './asset/halle/phaser-json-array/halle.png', './asset/halle/phaser-json-array/halle.json');
    };
})(window);
